...
def enroll():
    ...
    # Only ONE check for form submission
    if form.validate_on_submit():
        room_id = form.room_number.data
        
        # 1. CHECK CAPACITY: Count students currently in this room
        current_student_count = Student.query.filter_by(room_id=room_id).count()
        
        # 2. THE GUARD CLAUSE
        MAX_CAPACITY = 4  # You can change this number later if needed
        
        if current_student_count >= MAX_CAPACITY:
            flash(f'Room {room_id} is full! Capacity is {MAX_CAPACITY}.', 'danger')
            return redirect(url_for('enroll')) # Reload page to let them pick another room

        # 3. If space exists, proceed to create student
        # We hardcode the picture since we removed the upload feature
        student = Student(
            name=form.name.data,
            fee=form.fee.data,
            room_id=room_id,
...
def admin_register():
    ...
    if form.validate_on_submit():
        ...
        if existing_admin:
            ...
            new_admin = Admin(name=form.name.data, username=form.username.data, email=form.email.data, password_hash=hashed_password)
            try:
...
def edit_student(student_id):
    ...
    # 2. Get all rooms (so we can show them in the dropdown)
    rooms = Room.query.all()

    # 3. Handle the Update (When 'Save' is clicked)
    if request.method == 'POST':
        student.name = request.form['name']
        student.fee = float(request.form['fee'])
        student.room_id = int(request.form['room_id'])
        
        db.session.commit()
        flash('Student updated successfully!', 'success')
        return redirect(url_for('students'))

    # 4. Show the form (GET request)
...
def issues():
    ...
    if form.validate_on_submit():
        new_issue = Issue(
            ...
            # created_at is automatic due to default=datetime.utcnow in your model
        )
        try:
            db.session.add(new_issue)
            db.session.commit()
            flash('Issue reported successfully!', 'success')
            return redirect(url_for('issues')) # clear the form by reloading
        except Exception as e:
            db.session.rollback()
            flash(f'Error adding issue: {e}', 'danger')

    # 2. Display the page (GET)
    all_issues = Issue.query.order_by(Issue.created_at.desc()).all() # Newest first
    return render_template('issues.html', form=form, issues=all_issues)

@app.route('/issue/resolve/<int:issue_id>')
@login_required
...
@app.route('/issue/delete/<int:issue_id>')
@login_required
